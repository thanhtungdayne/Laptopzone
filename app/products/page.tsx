...
import type { Laptop, FilterState } from "@/types/product";
export default function ProductsPage() {
  ...
  return url.replace(/_AC_[^_]+_\.jpg/, ".jpg");
};
  ...
  axios.get(`${process.env.NEXT_PUBLIC_API_URL}/product`)
    .then((res) => {
      ...
      const mapped = res.data.result.map((item: any) => ({
        ...
        processor: item.processor?.join(", "),
        ram: item.ram,
        storage: item.storage,
        display: item.display,
        graphics: item.graphics?.join(", "),
        stock: item.stock,
 ...
 const filteredAndSortedLaptops = useMemo(() => {
  ...
  //   return true;
  // });
  const filtered = laptops.filter(() => true);
  ...
  return filtered;
}, [filters, sortBy, laptops]); // ← thêm `laptops` vào đây
